...
eqExp = go
  where
    ...
    go (Lam t1 b1) (Lam t2 b2) = do HRefl <- eqTypeRep t1 t2
                                    Refl <- go b1 b2
                                    return Refl
...
hashDeBruijn :: forall n (x :: Type) (xs :: Ctx n). Int -> Elem xs x -> SNat n -> Int
...
eqExpB (Lam ty1 body1) (Lam ty2 body2) | Just HRefl <- ty1 `eqTypeRep` ty2
                                       = body1 `eqExpB` body2
                                       | otherwise
                                       = False
...
exprType ctx (Var v) = go v ctx
  where
    go :: forall (v :: Vec a n) (x :: a). Elem v x -> SVec v -> Sing x
    go EZ     (h :%> _) = h
    go (ES e) (_ :%> t) = go e t
...
instance forall (ctx :: Ctx n) ty. SingI n => PrettyExp (Exp ctx ty) where
