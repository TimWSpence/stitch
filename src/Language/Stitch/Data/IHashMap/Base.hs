...
instance (forall i. (NFData (k i), NFData (v i))) => NFData (Leaf k v) where
  rnf (L k v) = rnf k `seq` rnf v

deriving instance (forall i. (Read (k i), Read (v i))) => Read (Leaf k v)
deriving instance (forall i. (Show (k i), Show (v i))) => Show (Leaf k v)
...
instance (forall i. (NFData (k i), NFData (v i))) => NFData (IHashMap k v) where
  rnf Empty                 = ()
...
instance (TestEquality k, IHashable k, forall i. (Read (k i), Read (e i)))
...
instance (forall i. (Show (k i), Show (v i))) => Show (IHashMap k v) where
